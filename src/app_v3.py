        ...
        if 'stock_recommendations' in st.session_state and st.session_state.stock_recommendations:
            ...
            for idx, rec in enumerate(recommendations, 1):
                display_data.append({
                    ...
                    'Urgency': rec['urgency'],
                    'Current Stock': f"{int(rec['current_stock'])}",
                    'Recommended Qty': f"{int(rec['recommended_qty'])}",
                    'Unit Cost': f"${int(rec['unit_cost']):,}",
                    'Unit Profit': f"${int(rec['unit_profit']):,}",
                    'ROI %': f"{int(rec['roi_percentage'])}%",
                    'Investment': f"${int(rec['total_investment']):,}",
                    'Expected Return': f"${int(rec['expected_return']):,}",
        ...
        if 'stock_recommendations' in st.session_state and st.session_state.stock_recommendations:
            ...
            with col2:
                ...
                if selected_recs:
                    ...
                    st.metric("Total Investment", f"${int(total_investment):,}")
                    st.metric("Expected Profit", f"${int(total_profit):,}")
                    st.metric("Average ROI", f"{int(avg_roi)}%")
                else:
...
def display_missing_products_analysis(master_df, selected_month, selected_year, selected_shape, selected_color, selected_bucket):
    ...
    try:
        ...
        with col4:
            st.metric(
                "Avg Months Missing",
                f"{int(stats['avg_months_missing'])}",
...
def normalize_bucket_names(df):
    """Normalize bucket names to merge 1.50-1.74 and 1.75-1.99 into 1.50-1.99"""
    try:
...
def display_missing_products_tab(master_df):
    ...
    try:
        ...
        with col1:
            months = ["None"] + sort_months_stable(tuple(master_df['Month'].unique()))
            selected_month = st.selectbox("Filter by Month", months, key="missing_month_filter")
        with col2:
            years = ["None"] + sorted(list(master_df['Year'].unique()))
            selected_year = st.selectbox("Filter by Year", years, key="missing_year_filter")
        with col3:
            shapes = ["None"] + sorted(list(master_df['Shape key'].unique()))
            selected_shape = st.selectbox("Filter by Shape", shapes, key="missing_shape_filter")
        with col4:
            colors = ["None"] + ['WXYZ', 'FLY', 'FY', 'FIY', 'FVY']
            selected_color = st.selectbox("Filter by Color", colors, key="missing_color_filter")
        with col5:
            buckets = ["None"] + sort_buckets(list(stock_bucket.keys()))
            selected_bucket = st.selectbox("Filter by Bucket", buckets, key="missing_bucket_filter")
...
def display_data_table(display_df):
    ...
    try:
        if not display_df.empty:
            ...
            # Exclude Shape, Color, Month, Year columns (keep Shape key and Color Key)
            columns_to_exclude = ['Shape', 'Color', 'Month', 'Year']
            columns_to_display = [col for col in display_df.columns if col not in columns_to_exclude]

            # Reorder columns: Place 'Average Cost (USD)' & 'Avg Cost Total' after 'Quarter'
            if 'Quarter' in columns_to_display:
                # Find position of Quarter
                quarter_index = columns_to_display.index('Quarter')

                # Remove cost columns from their current positions if they exist
                cost_columns = []
                if 'Average Cost (USD)' in columns_to_display:
                    columns_to_display.remove('Average Cost (USD)')
                    cost_columns.append('Average Cost (USD)')
                if 'Avg Cost Total' in columns_to_display:
                    columns_to_display.remove('Avg Cost Total')
                    cost_columns.append('Avg Cost Total')

                # Insert cost columns right after Quarter
                for i, cost_col in enumerate(cost_columns):
                    columns_to_display.insert(quarter_index + 1 + i, cost_col)
